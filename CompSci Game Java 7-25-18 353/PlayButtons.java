...
public class PlayButtons extends JPanel{
   ...
   private class StarttListener implements ActionListener
   {
      public void actionPerformed(ActionEvent e)
      {
         ...
         prevpanel.removeAll();
         try {
            if(OptionButton.soundon.getIcon() != OptionButton.sOn && OptionButton.soundon.getIcon() != OptionButton.sOff)
            {
               try{
                  JPanel newpanel = new GamePanel(livespassed,false);
               ...
               try{
                  JPanel newpanel = new GamePanel(livespassed,OptionButton.getSoundStatus());
                  newpanel.setPreferredSize(new Dimension(1920,1080));
                  prevpanel.add(newpanel);
                  AstroDriver.frame.repaint();
                  AstroDriver.frame.setVisible(true);
         ...
         {
            try{
               JPanel newpanel = new GamePanel(livespassed,true);
               newpanel.setPreferredSize(new Dimension(1920,1080));
               prevpanel.add(newpanel);
               AstroDriver.frame.repaint();
               AstroDriver.frame.setVisible(true);
